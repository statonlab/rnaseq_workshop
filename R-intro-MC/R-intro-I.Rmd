...
pbsc
```
You might see the `<-` assignment in people's code.  It is the same as `=`.
...
* `[` is similar to `[[`. The difference is that `[[` can only return a single value, while `[[` can return multiple values.
                         ...
                         Here when you talk about difference, you may forget to change the first'[[' to '['.
                          Another thing is: when I type in pbsc[[3]] it also gives multiple numbers, while pbsc[3] gives the 3rd list.                            When I read through the below material, I realized the difference occured when extracting elements from a vector  
```{r}
pbsc
```
...
my_template
my_template['a'] 
my_template[c('g','g','a','c')] # I added these two lines and hope it will help them to understand how this worked for the below code. I am not sure what do you feel. It helps me to understand:)
...
When you import data from a file, R treats string columns as factors by default.
