...
# The RStudio and RStudio interface



# Set working directory
First, lets download the data for this class: https://github.com/statonlab/rnaseq_workshop/raw/master/R-intro-MC.zip. You can put the file on your destkop and unzip the file. The path to your data folder should be `~/Desktop/R-intro-MC` for Mac users or `C:/Users/your-user-name/Desktop/R-intro-MC` for windows users.

R has a working directory, which is like being in a certain location in the terminal. That location is where R will automatically look for files and write new files. You can find out where this location is with `getwd()`.
```{r eval=FALSE}
getwd()
```

You can also set to a new location, anywhere in your file system. We want this to be the data folder that just downloaded to your Desktop for now. We can do this through the R command.
```{r eval=FALSE}
# For Mac user
setwd('~/Desktop/R-intro-MC')
# For Windows user (Thanks to Fang!)
setwd('C:/Users/yourusername/Desktop/R-intro-MC')
```

Also, you can set the working directory through RStudio by clicking **Session->Set working directory->Choose directory**.
...
We have two functions to attach a package to your R session. The `library()` function returns error if the package loading fails, while the `require()` functions return a logical value FALSE if the package loading fails.

```{r}
library('ggplot2')
require('ggplot2')

# when loading a package that is not installed or does not exit.
library('ggplot22') # returns an error
require('ggplot22') # returns some warning messages.
```
...
* find the **Git source** link at the bottom of the page.
...
* The `swirl()` package is an interactive tutorial for learning R 
    + swirl homepage: http://swirlstats.com/
    + courses list: https://github.com/swirldev/swirl_courses#swirl-courses
...
## Built-in datasets
These datasets are available to all R installations or come with the package you installed, they are great for learning and practicing.
```{r}
data(package='base')
...
Both `read.csv()` and `read.table()` read a flat file (e.g., csv file or delimited file) in table format. *By default*, `read.csv()` reads comma separated values and `read.table()` reads *space or tab* separated values. But you can change the default and use these two functions interchangeably.
...
* Put one of your own csv, txt or gtf files into the working directory and try to read the data into R with these two functions.

<hr>

## variables

We have loaded data into the memory, but we can't access the data. Because we didn't assign the data to a name. When we assign a value to a name (like `x = 5`), we create a variable.
...
my_v1$A # this won't work! Because '$' does not work for atomic vectors!
```
...
# how to convert lowercase bases to uppercase bases?
# we create an uppercase base vector and name each element with the corresponding lowercase letter.
my_mapping = c('G','C','T','A')
names(my_mapping) = c('g', 'c', 't', 'a')
my_mapping

# we select elements from the uppercase vector by using the lowercase vector as the selecting vector.
my_mapping['a'] 
my_mapping[c('g','g','a','c')] 

my_mapping[my_sequence] # yeah!
```
...
The workspace is your current working environment which includes all the variables, objects and functions you have already created or loaded. You can save this environment into a file. When you load the file into R next time, all the variables, objects and functions will be avaiable to you.

Two ways to save workspace:
* Save through RStudio: go to **Session->Save Workspace->Choose Directory**
* Save through R command: `save.image('my-workspace.RData')`

To load workspace to RStudio, you can click **Session->Load Workspace**
