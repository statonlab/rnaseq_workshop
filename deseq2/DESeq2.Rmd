...
setwd("E:/UT/rnaseq_ws/2017/deseq")
library(DESeq2)
```
...
### Count data files
Our output files from HTSeq-count indicate genes on the first column and gene counts on the second column. This is what they look like.
...
myDESeqData
```
The `design` parameter looks simple enough in this case, but it is *critical* to understand your experimental design.  Remember you can run `?DESeqDataSetFromHTSeqCount` for help on functions and their parameters.
...
* P value is for a single test (meaning, every transcript gets its own test)
...
To complement the differential expression results, annotations are added to each transcript.
```{r}
...
myRes.anno <- merge(as.data.frame(myRes.sig), anno, by.x="row.names", by.y="locusName", all = TRUE, all.y = FALSE)
...
vsd.heatmap = assay(vsd.myDESeqData[rownames(myRes.sig[order(myRes.sig$padj), ])[1:50], ])

library(pheatmap)
pheatmap(rl.heatmap)
pheatmap(vsd.heatmap)
```

### Heatmap of the sample-to-sample distances
The distance between samples can be calculated using the `dist()` function on the transpose of the transformed count matrix. The heatmap shows the similarities between samples.
```{r}
...
This is another option to visualize the distance between samples.

```{r}
plotPCA(rl.myDESeqData[rownames(myRes.sig), ], intgroup="pheno")
plotPCA(vsd.myDESeqData[rownames(myRes.sig), ], intgroup="pheno")
```

## Try it yourself 
### Significant changes in expression
* We are typically interested in the transcripts that are most significantly up or down-regulated.  Return to the section *p value, adjusted p value (padj)  and alpha* for the below exercises.
  - Try subsetting the data so that you have two objects: one containing all the significantly upregulated transcripts, the other the downregulated transcripts.
* How can you summarize this information?
  - the total number of genes in each set?
  - the range of p-values?
  - the average fold change?

### Looking at specific gene families
* Often, you will be interested in a specific gene family.  Your lab might study transporters or kinases and want to look at that data seperately.
* Examine the annotations.  Try picking 1 or multiple GO terms that you think are interesting for this research project.
  - Subset your data to just include genes with these GO terms.
  - Rerun the below analyses.  How did the results change?
*  In the above try it yourself, we subsetted the data to only include significantly up and down regulated transcripts.
  - Make a table summarizing the annotations for up and down-regulated transcripts.
